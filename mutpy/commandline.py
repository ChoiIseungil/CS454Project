...
# def main(argv):
#     parser = build_parser()
#     return run_mutpy(parser)

def main(target, unit_test):
    return run_mutpy([target], [unit_test])
...
def run_mutpy(target, unit_test):
    mutation_controller = build_controller(target, unit_test)
    return mutation_controller.run()
...
def build_controller(target, unit_test):
    from mutpy.test_runners import UnittestTestRunner
    runner_cls = UnittestTestRunner
    built_views = [views.QuietTextView(None)]
    mutant_generator = build_mutator()
    target_loader = utils.ModulesLoader(target, None)
    test_loader = utils.ModulesLoader(unit_test, None)
...
def build_mutator():
    ...
    operator_set |= operators.standard_operators
    return controller.FirstOrderMutator(operator_set, 100)
