...
def processing(generation, best_input, best_value, fun_name, k, fitness_step, evaluator):
    fitnesses_results = []
...
def pga(population, mutation_rate, fun_name, n, m, k, fitness_step, evaluator):
    generation_step = 0
    best_value = 0
    ...
    while generation_step < 2500:
        ...
        for index, population in enumerate(generation):
            ...
            for i in range(m):
                ...
                fitnesses_result_, fitness_step = fitnesses(population_, best_value, fun_name, fitness_step, evaluator)
                fitnesses_result += fitnesses_result_
                population = population + population_
                new_population = step(population, fitnesses_result, population_size, fun_name, mutation_rate)
                new_generation.append(new_population)
        generation, fitnesses_results, best_input, best_value, fitness_step = processing(new_generation, best_input, best_value, fun_name, k, fitness_step, evaluator)
...
def main(population, mutation_rate, fun_name, n, m, k, evaluator):
    start = time.time()
    best_input, best_value, fitness_step, total_population_size = pga(population, mutation_rate, fun_name, n, m, k, 0, evaluator)
